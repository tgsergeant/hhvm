...
void emitIncRef(Asm& as, PhysReg base) {
  ...
  // emit incref
  as.incb(base[FAST_REFCOUNT_OFFSET]);
  if (RuntimeOption::EvalHHIRGenerateAsserts) {
...
void emitIncRefGenericRegSafe(Asm& as, PhysReg base, int disp, PhysReg tmpReg) {
  { // if RC
    ...
    { // if !static
      ...
      as. incb(tmpReg[FAST_REFCOUNT_OFFSET]);
    } // endif
  } // endif
}
...
void emitAssertRefCount(Asm& as, PhysReg base) {
  as.cmpb(HPHP::StaticValue, base[FAST_REFCOUNT_OFFSET]);
  ifThen(as, CC_NLE, [&] {
    as.cmpb(HPHP::RefCountMaxRealistic, base[FAST_REFCOUNT_OFFSET]);
